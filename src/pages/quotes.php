...
if (!isset($_GET['page']) OR empty($_GET['page']) OR !is_numeric($_GET['page']) OR is_float($_GET['page']) OR $_GET['page'] > $pagesTotal OR $_GET['page'] <= 0) $page = 1;
else $page = $_GET['page'];

switch ($_GET['sort']) {
	case 'former':
		$title = "Premières quotes";
		$content['title'] = 'Premères quotes - Page '.$page;
		$SQLrequest = "SELECT * FROM `DTK` WHERE approved = 1 ORDER BY DTK.id ASC";
		break;
	case 'new':
		$title = "Quotes les plus récentes";
		$content['title'] = 'Quotes récentes - Page '.$page;
		$SQLrequest = "SELECT * FROM `DTK` WHERE approved = 1 ORDER BY DTK.date DESC";
		break;
	case 'old':
		$title = "Quotes les plus vieilles";
		$content['title'] = 'Quotes vieilles - Page '.$page;
		$SQLrequest = "SELECT * FROM `DTK` WHERE approved = 1 ORDER BY DTK.date ASC";
		break;
	case 'voice':
		$title = "Quotes de chat vocal";
		$content['title'] = 'Quotes vocales - Page '.$page;
		$SQLrequest = "SELECT * FROM `DTK` WHERE approved = 1  AND `source-type` = 1 ORDER BY DTK.id DESC";
		break;
	case 'text':
		$title = "Quotes de chat textuel";
	...
	case 'name':
		...
		break;
	default:
		$title = "Dernières quotes";
		$content['title'] = 'Dernières quotes - Page '.$page;
...
if ($pagePrevious > 0) $nav .= "<a href=\"".$pagePrevious."\" class=\"nav-button\" style=\"float:left\">❮ Page ".$pagePrevious."</a>";
if ($pageNext <= $pagesTotal) $nav .= "<a href=\"".$pageNext."\" class=\"nav-button\" style=\"float:right;\">Page ".$pageNext." ❯</a>";
$nav .= "</div>";

$quotesHTML = "<h2 style=\"text-align:center;margin:0\">".$title."</h2><div class=\"quotes\" id=\"quo\">";
